 ...
 * @author TmpAdmin
 */
public class SendPublicFileWindow extends Stage{
    private final TextField textFieldPath = new TextField();
    private final Label errorLabel = new Label();
    
    private final FXMLStartPageController controller;
    private static SendPublicFileWindow singleton;
    
    private SendPublicFileWindow(FXMLStartPageController controller){
        this.controller = controller;
    }
    
    public static SendPublicFileWindow getSingleton(FXMLStartPageController controller){
        if (singleton == null){
            singleton = new SendPublicFileWindow(controller);
    ...
    public void initialize(Stage primaryStage){
        ...
        System.out.println(SendPublicFileWindow.class);
        InputStream streamBrowse = SendPublicFileWindow.class.getResourceAsStream("browse.png");
        browseButton.setGraphic(new ImageView(new Image(streamBrowse)));
    ...
    private void send(){
        ...
        if (path.equals(""))
            ...
            try {
                FXMLStartPageController.client.sendFile(controller.getCurrentRoomName(), path);
